...
const Motocross = ({ products, isLoading, error }) => {
  const MotocrossProducts = products.filter((product) => product.category === "motocross");

  return (
    <div className="px-4 py-6">
      <h2 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6 text-center">Motocross</h2>

      {isLoading ? (
        <div className="flex flex-col items-center justify-center h-64">
          <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-gray-900 mb-4"></div>
          <p>Loading Motocross gear...</p>
        </div>
      ) : error ? (
          ...
          {MotocrossProducts.map((product, i) => (
            <div key={i} className="bg-white p-2 sm:p-4">
              <Link to={`/product/${product._id}`} className="block">
                ...
                <p className="text-xs sm:text-sm text-gray-600">Motocross Helmet</p>
                <h3
