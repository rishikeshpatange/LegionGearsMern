...
const Racing = ({ products, isLoading, error }) => {
  ...
  return (
    <div className="px-4 py-6">
      <h2 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6 text-center">Racing</h2>
          ...
          {racings.map((racing, i) => (
            <div key={i} className="bg-white p-2 sm:p-4">
              <Link to={`/product/${racing._id}`} className="block">
                ...
                <p className="text-xs sm:text-sm text-gray-600">Racing Helmet</p>
                <h3
                ...
                <p className="font-medium text-sm sm:text-base">₹{racing.price}</p>
              </Link>
            </div>
          ))}
        </div>
